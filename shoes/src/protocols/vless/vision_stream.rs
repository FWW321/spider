...
pub struct VisionStream<IO> {
    ...
    /// Reusable buffer for TLS read operations (TCP 锟?read_tls 锟?decrypt)
    /// Used in PaddingTls mode
    ...
    IO: AsyncRead + AsyncWrite + Unpin,
{
    ...
    /// Drain all pending writes in PaddingTls mode: plain 锟?TLS 锟?TCP
    ///
    /// This drains the write pipeline in priority order:
    /// 1. TLS session 锟?TCP socket (highest priority, creates space in session)
    /// 2. Plain buffer 锟?TLS session (writes to session)
    ///
    ...
    fn poll_read_padding_tls(
        ...
        loop {
            ...
            loop {
                ...
                let mut i = 0;
                while i < plaintext_len {
                    let decrypted_part: &[u8] = match reader.fill_buf() {
                        Ok(buf) => buf,
                        Err(e) => {
                ...
                while let Some(record) = outer_read_deframer.next_record()? {
                    ...
                    if plaintext_len > 0 {
                        ...
                        while total_read < plaintext_len {
                            let chunk: &[u8] = match reader.fill_buf() {
                                Ok(buf) => buf,
                                Err(e) => {
    ...
    IO: AsyncRead + AsyncWrite + Unpin,
{
    fn poll_read(
        ...
        if !this.pending_read.is_empty() {
            let len: usize = buf.remaining().min(this.pending_read.len());
...
impl<IO> crate::network::async_stream::AsyncPing for VisionStream<IO>
where
    IO: AsyncStream,
{
