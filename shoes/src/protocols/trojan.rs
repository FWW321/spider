...
use crate::protocols::shadowsocks::{
    DefaultKey, ShadowsocksCipher, ShadowsocksKey, ShadowsocksStream, ShadowsocksStreamType,
};
use crate::protocols::socks::{CMD_CONNECT, CMD_UDP_ASSOCIATE, read_location, write_location_to_vec};
use crate::network::stream_reader::StreamReader;
use crate::network::tcp::tcp_handler::{
    TcpClientHandler, TcpClientSetupResult, TcpServerHandler, TcpServerSetupResult,
};
use crate::utils::common::write_all;
...
impl TcpClientHandler for TrojanTcpHandler {
    async fn setup_client_tcp_stream(
        ...
        // Combine header into a single buffer to ensure it's sent in a single TLS record/packet.
        // Format: password_hash + CRLF + command + address + CRLF
        let location_bytes = write_location_to_vec(&remote_location);
        let mut header = Vec::with_capacity(self.password_hash.len() + 2 + 1 + location_bytes.len() + 2);
        header.extend_from_slice(&self.password_hash);
        header.extend_from_slice(&CRLF_BYTES);
        header.push(CMD_CONNECT);
        header.extend_from_slice(&location_bytes);
        header.extend_from_slice(&CRLF_BYTES);

        write_all(&mut client_stream, &header).await?;
