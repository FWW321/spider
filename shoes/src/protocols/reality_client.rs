...
// 2. Avoids a dependency cycle: crypto 鈫?reality (core types), but this handler 鈫?crypto
...
use crate::protocols::reality::{CipherSuite, RealityClientConfig, RealityClientConnection};
use crate::network::tcp::tcp_handler::{TcpClientHandler, TcpClientSetupResult};
    ...
    async fn setup_client_tcp_stream(
        ...
        match self.handler {
            ...
            RealityInnerClientHandler::VisionVless { ref uuid, .. } => {
                crate::protocols::vless::vless_client_handler::setup_custom_tls_vision_vless_client_stream(
                    tls_stream,
                    uuid,
                    &remote_location,
                )
    ...
    async fn setup_client_udp_bidirectional(
        ...
        match &self.handler {
            ...
            RealityInnerClientHandler::VisionVless { uuid, .. } => {
                crate::protocols::vless::vless_client_handler::setup_vless_udp_bidirectional(
