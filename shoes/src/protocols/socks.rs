...
use crate::network::socks5_udp_relay::Socks5UdpRelayStream;
...
async fn handle_udp_associate(
    ...
    let udp_socket: std::net::UdpSocket =
        match crate::network::socket_util::new_socket2_udp_socket_with_buffer_size(
            bind_ip.is_ipv6(),
            None,
            Some(udp_bind_addr),
            false,
...
///
/// This function monitors the TCP connection for termination.
/// When the TCP connection closes, the UDP relay is terminated.
/// The UDP relay stream handles packets directly.
async fn run_udp_associate(
    mut tcp_stream: Box<dyn AsyncStream>,
    _relay_stream: Socks5UdpRelayStream,
    _proxy_selector: Arc<ClientProxySelector>,
    _resolver: Arc<dyn Resolver>,
) -> std::io::Result<()> {
    // Simply monitor the TCP connection for closure
    // The relay stream handles UDP packets internally
    monitor_tcp_close(&mut tcp_stream).await
}
