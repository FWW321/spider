    ...
    IO: AsyncStream,
{
    fn poll_write(
        ...
        // 1. Always try to drain any existing TLS data first.
        // This is crucial! If there's pending ciphertext from a previous write,
        // we must send it before trying to encrypt more plaintext.
        while self.session.wants_write() {
            match self.write_tls_direct(cx) {
                ...
                Poll::Pending => {
                    // Underlying transport is full. We can't proceed with more application data.
                    return Poll::Pending;
                }
        ...
        // 2. Write new plaintext to session
        let n = match self.session.writer().write(buf) {
            Ok(n) => n,
            Err(e) => return Poll::Ready(Err(e)),
        };

        // 3. Immediately try to drain the newly encrypted data.
        // We don't return Pending here even if it blocks, because we've successfully
        // buffered the data in the session. However, we set need_flush.
        while self.session.wants_write() {
            match self.write_tls_direct(cx) {
                Poll::Ready(Ok(0)) => return Poll::Ready(Err(io::ErrorKind::WriteZero.into())),
                Poll::Ready(Ok(_)) => {
    ...
    fn poll_flush(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<io::Result<()>> {
        // Flush the session writer first
