        ...
        TcpServerSetupResult::TcpForward {
            ...
            if let Some(mut client_stream) = client_stream {
                if let Some(resp) = connection_success_response {
                    crate::util::write_all(&mut stream, &resp).await?;
                }
                if let Some(data) = initial_remote_data {
                    crate::util::write_all(&mut client_stream, &data).await?;
                }
...
async fn run_tcp_server_loop(
    ...
    let listener = crate::socket_util::new_tcp_listener(addr, 1024, None)?;
