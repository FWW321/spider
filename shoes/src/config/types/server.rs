...
// Forward declarations for client types
...
"#;
    ...
    fn test_shadowtls_handshake_serialization() {
        ...
        let yaml = serde_yml::to_string(&local).unwrap();
        ...
        let yaml = serde_yml::to_string(&remote).unwrap();
...
"#;
        ...
        let result: Result<ShadowTlsServerHandshakeConfig, _> =
            serde_yml::from_str(invalid_missing_key);
        assert!(result.is_err());
...
"#;
        ...
        let result: Result<ShadowTlsServerHandshakeConfig, _> = serde_yml::from_str(invalid_mixed);
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
...
"#;
        ...
        let result: Result<ShadowTlsServerHandshakeConfig, _> =
            serde_yml::from_str(invalid_unknown);
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
...
"#;
        ...
        let result: Result<ShadowTlsServerHandshakeConfig, _> = serde_yml::from_str(valid_local);
        assert!(result.is_ok());
        assert!(matches!(
            result.unwrap(),
...
"#;
        ...
        let result: Result<ShadowTlsServerHandshakeConfig, _> = serde_yml::from_str(valid_remote);
        assert!(result.is_ok());
        assert!(matches!(
            result.unwrap(),
...
"#;
        ...
        let result: Result<ShadowTlsServerHandshakeConfig, _> =
            serde_yml::from_str(invalid_remote_syntax);
        assert!(result.is_err());
        let error_msg = result.unwrap_err().to_string();
...
bad_option: "value"
"#;
        let result: Result<ShadowTlsServerHandshakeConfig, _> = serde_yml::from_str(yaml);
        assert!(result.is_err());
        let err = result.unwrap_err().to_string();
        assert!(
...
address: "example.com:443"
"#;
        let result: Result<ShadowTlsRemoteHandshake, _> = serde_yml::from_str(yaml);
...
client_chain:
  - address: "proxy.example.com:1080"
    protocol:
      type: socks
"#;
        let original: ShadowTlsRemoteHandshake = serde_yml::from_str(yaml).unwrap();
        let serialized = serde_yml::to_string(&original).unwrap();
        println!("Serialized:\n{serialized}");
        let deserialized: ShadowTlsRemoteHandshake = serde_yml::from_str(&serialized).unwrap();
