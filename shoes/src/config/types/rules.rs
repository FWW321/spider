                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                if let Some(chain_value) = mapping.get(&chain_key) {
                    ...
                    let hops: OneOrSome<ClientChainHop> =
                        serde_yml::from_value(chain_value.clone()).map_err(|e| {
                            Error::custom(format!(
                ...
                let proxy: ClientConfig = serde_yml::from_value(value).map_err(|e| {
                    Error::custom(format!(
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                if let Some(pool_value) = map.get(&pool_key) {
                    ...
                    let selections: OneOrSome<ConfigSelection<ClientConfig>> =
                        serde_yml::from_value(pool_value.clone()).map_err(|e| {
                            Error::custom(format!(
...
action: block
"#;
        let result: Result<RuleConfig, _> = serde_yml::from_str(yaml);
        assert!(result.is_ok());
...
- exit-proxy
"#;
        let result: Result<ClientChain, _> = serde_yml::from_str(yaml);
        assert!(result.is_ok());
...
chain:
  ...
  - exit-proxy
"#;
        let result: Result<ClientChain, _> = serde_yml::from_str(yaml_chain);
        assert!(result.is_ok());
        let chain = result.unwrap();
...
protocol:
  type: socks
"#;
        let result: Result<ClientChain, _> = serde_yml::from_str(yaml_proxy);
        assert!(result.is_ok());
        let chain = result.unwrap();
...
"#;
    ...
    fn test_client_chain_hop_single_group_ref() {
        let yaml = r#"my-proxy-group"#;
        let result: Result<ClientChainHop, _> = serde_yml::from_str(yaml);
        assert!(result.is_ok());
        assert!(matches!(
            result.unwrap(),
...
"#;
    ...
    fn test_chain_hop_serialization_roundtrip_single_group() {
        ...
        let deserialized: ClientChainHop = serde_yml::from_str(&yaml).unwrap();
        assert!(matches!(
            deserialized,
            ClientChainHop::Single(ConfigSelection::GroupName(_))
        ));
    ...
    fn test_client_chain_serialization_roundtrip() {
        ...
        let deserialized: ClientChain = serde_yml::from_str(&yaml).unwrap();
